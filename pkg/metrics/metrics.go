...
var (
	...
	demandFungibilityExtractedTotal = k8smetrics.NewCounterVec(
		&k8smetrics.CounterOpts{
			Namespace: caNamespace,
			Name:      "demand_fungibility_impact_tracking_id_extracted_total",
			Help:      "Tracks when a Recommendation ID is successfully parsed from an advisory API.",
		},
		[]string{"source"},
	)

	demandFungibilityInjectedTotal = k8smetrics.NewCounterVec(
		&k8smetrics.CounterOpts{
			Namespace: caNamespace,
			Name:      "demand_fungibility_impact_tracking_id_injected_total",
			Help:      "Tracks when an ID is successfully injected into a CreateInstances request.",
		},
		[]string{"source"},
	)

	demandFungibilityMissingIdTotal = k8smetrics.NewCounterVec(
		&k8smetrics.CounterOpts{
			Namespace: caNamespace,
			Name:      "demand_fungibility_impact_tracking_missing_id_total",
			Help:      "Tracks edge cases where an expected identifier was lost or unparseable.",
		},
...
const (
	...
	// CSNPodsLifecycleProcessorError represents an unexpected error in the CSN pods lifecycle processor.
	CSNPodsLifecycleProcessorError CSNInvalidCondition = "CSNPodsLifecycleProcessorError"
	// CSNBufferConsumptionProcessorError represents an unexpected error in the CSN buffer consumption processor.
	CSNBufferConsumptionProcessorError CSNInvalidCondition = "CSNBufferConsumptionProcessorError"
	// CSNNodeReconciliationProcessorError represents an unexpected error in the CSN node reconciliation processor.
	CSNNodeReconciliationProcessorError CSNInvalidCondition = "CSNNodeReconciliationProcessorError"
	// CommitSnapshotError represents an error during committing snapshot.
	CommitSnapshotError CSNInvalidCondition = "CommitSnapshotError"
)
...
// DemandFungibilitySource represents the source of an advisory API.
type DemandFungibilitySource string

const (
	// FA represents Flex Advisor.
	FA DemandFungibilitySource = "FA"
	// RLA represents Resource Location Advisor.
	RLA DemandFungibilitySource = "RLA"
)

// RegisterDemandFungibilityExtracted registers an extracted demand fungibility occurrence.
func (*prometheusMetrics) RegisterDemandFungibilityExtracted(source DemandFungibilitySource) {
	demandFungibilityExtractedTotal.WithLabelValues(string(source)).Inc()
}

// RegisterDemandFungibilityInjected registers an injected demand fungibility occurrence.
func (*prometheusMetrics) RegisterDemandFungibilityInjected(source DemandFungibilitySource) {
	demandFungibilityInjectedTotal.WithLabelValues(string(source)).Inc()
}

// RegisterDemandFungibilityMissingId registers a missing id demand fungibility occurrence.
func (*prometheusMetrics) RegisterDemandFungibilityMissingId(source DemandFungibilitySource, reason string) {
	demandFungibilityMissingIdTotal.WithLabelValues(string(source), reason).Inc()
}

...
func (b *Builder) Build(
	...
	var updatesCh chan status.UpdateMessage
	if autoscalingOptions.EnhancedCrdStatusReporting {
		updatesCh = make(chan status.UpdateMessage, 2000)
		...
		aggregator := status.NewAggregator(b.npcCrdClient, b.npcCrdLister, updatesCh, ctrClient)
	...
	if autoscalingOptions.EnhancedCrdStatusReporting {
		cc_history.SetupHistoryResetObserver(cccInformer, updatesCh)
	}

	var minCapacityObserver cc_processors.MinCapacityObserver
	if autoscalingOptions.EnableComputeClassMinCapacity {
		minCapacityObserver = cc_processors.NewMinCapacityObserver(internalmetrics.Metrics, updatesCh)
	...
	if autoscalingOptions.EnhancedCrdStatusReporting {
		inf := npc_history.NewCrdBackoffObserver(updatesCh, b.npcCrdLister, cloudProvider)
	...
	if autoscalingOptions.GCEFlexAdvisorEnabled {
		flexAdvisor, err = flexadvisor.NewFlexAdvisor(bgContext, cloudProvider, b.npcCrdLister, cloudProvider, b.optsTracker)
		if err != nil {
	...
	// Create autoscaler.
	autoscaler, err := core.NewAutoscaler(bgContext, opts, informerFactory)
	if err != nil {

...
func (ns *throttledNoScaleUp) computeSkippedMigs(scaleUpStatus *vistypes.ScaleUpStatus) []*vistypes.MigExplanation {
	// If simulation is enabled, skipped MIGs are reported per pod group instead of globally.
	if ns.simulationForSkippedMigsEnabled {
		return nil
	}
...
func (ns *throttledNoScaleUp) computePodGroups(scaleUpStatus *vistypes.ScaleUpStatus, napStatus *vistypes.NapStatus) []*vistypes.PodGroupExplanation {
	...
	for _, noScaleUpInfo := range scaleUpStatus.NoScaleUpInfos {
		rejectedMigReasons := ns.computePodLevelRejectedMigReasons(noScaleUpInfo, migsById)
		skippedMigReasons := ns.computePodLevelSkippedMigReasons(noScaleUpInfo, migsById)
		napReasons := ns.computePodLevelNapReasons(noScaleUpInfo, napStatus, migsById)
		pgm.AddPod(noScaleUpInfo.Pod, rejectedMigReasons, napReasons, skippedMigReasons)
	}
	return pgm.GetPodGroups()
}

func (ns *throttledNoScaleUp) computePodLevelSkippedMigReasons(noScaleUpInfo *vistypes.NoScaleUpInfo, migsById map[string]*vistypes.GkeMig) map[string]*vistypes.MigExplanation {
	// If simulation is enabled, skipped MIGs are reported per pod group and we collect them here.
	if !ns.simulationForSkippedMigsEnabled {
		return nil
	}
	result := make(map[string]*vistypes.MigExplanation, len(noScaleUpInfo.SkippedNodeGroups))
	for migId, reasons := range noScaleUpInfo.SkippedNodeGroups {
		mig, found := migsById[migId]
		if !found {
			klog.Errorf("CA Viz NoScaleUp: Mig %s marked as skipped in ScaleUpResult, but wasn't provided in ConsideredNodeGroups.", migId)
			continue
		...
		result[mig.Id] = &vistypes.MigExplanation{
			Mig:    mig,
			Reason: vistypes.NewNoScaleUpMigSkippedMsg(reasons.Reasons()),
		}
	}
...
func (ns *throttledNoScaleUp) computePodLevelRejectedMigReasons(noScaleUpInfo *vistypes.NoScaleUpInfo, migsById map[string]*vistypes.GkeMig) map[string]*vistypes.MigExplanation {

		...
		PodGroups: []*vistypes.PodGroupExplanation{
			{
				...
				MigReasons: map[string]*vistypes.MigExplanation{},
				NapReasons: []*vistypes.Message{},
				SkippedMigReasons: map[string]*vistypes.MigExplanation{
					"mig1": {Mig: mig1, Reason: vistypes.NewNoScaleUpMigSkippedMsg([]string{"Reason1", "Reason2"})},
				},
			},
		},
	}, reasons)
...
func TestSkippedMigReasons(t *testing.T) {
	...
	// skippedReasons are nil if the skipped MIGs simulation feature is disabled.
	noScaleUpWithSimulation := throttledNoScaleUp{simulationForSkippedMigsEnabled: true}
	skippedReasons = noScaleUpWithSimulation.computeSkippedMigs(&vistypes.ScaleUpStatus{NoScaleUpInfos: []*vistypes.NoScaleUpInfo{noScaleUpInfo}, ConsideredMigs: consideredMigs})
	assert.Nil(t, skippedReasons)
}

func TestPodLevelSkippedMigs(t *testing.T) {
	migsById := []*vistypes.GkeMig{
		{Id: "mig1", Name: "mig1", NodePoolName: "np1", Zone: "z1", Exists: true},
		{Id: "mig2", Name: "mig2", NodePoolName: "np2", Zone: "z1", Exists: true},
		{Id: "mig3", Name: "mig3", NodePoolName: "np3", Zone: "z1", Exists: false},
	}
	pod := &vistypes.Pod{Name: "pod1"}
	...
	scaleUpStatus := &vistypes.ScaleUpStatus{
		ConsideredMigs: migsById,
		NoScaleUpInfos: []*vistypes.NoScaleUpInfo{noScaleUpInfo},
	}

	noScaleUp := throttledNoScaleUp{}

	// Case 1: Simulation is disabled. Pod groups level SkippedMigs are nil .
	podGroups := noScaleUp.computePodGroups(scaleUpStatus, &vistypes.NapStatus{})
	assert.Equal(t, 1, len(podGroups))
	assert.Nil(t, podGroups[0].SkippedMigReasons)

	// Case 2: Simulation is enabled. Pod groups should have populated SkippedMigs.
	noScaleUpWithSimulation := throttledNoScaleUp{simulationForSkippedMigsEnabled: true}
	podGroups = noScaleUpWithSimulation.computePodGroups(scaleUpStatus, &vistypes.NapStatus{})
	assert.Equal(t, 1, len(podGroups))
	assert.Equal(t, map[string]*vistypes.MigExplanation{
		"mig1": {Mig: migsById[0], Reason: vistypes.NewNoScaleUpMigSkippedMsg([]string{"Reason1", "Reason2"})},
		"mig2": {Mig: migsById[1], Reason: vistypes.NewNoScaleUpMigSkippedMsg([]string{"Reason3"})},
	}, podGroups[0].SkippedMigReasons)
}
...
func TestPodGroupReasons(t *testing.T) {
	...
	noScaleUp := throttledNoScaleUp{}
	migReasons := noScaleUp.computePodLevelRejectedMigReasons(noScaleUpInfo, migsById)
	assert.Equal(t, map[string]*vistypes.MigExplanation{

...
// Reasons contains all possible reasons that can cause the scale-up not to be performed.
type Reasons struct {
	TopLevel    *vistypes.Message
	TopLevelNap *vistypes.Message
...
func (r *Reasons) IsEmpty() bool {
	return r.TopLevel == nil && r.TopLevelNap == nil && len(r.SkippedMigs) == 0 && len(r.PodGroups) == 0
}

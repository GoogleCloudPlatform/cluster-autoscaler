...
type PodGroupExplanation struct {
	...
	NapReasons []*Message
	// SkippedMigReasons contains reasons why particular MIGs were skipped for this group.
	SkippedMigReasons map[string]*MigExplanation
}
...
func (pg *PodGroupExplanation) Proto() *vispb.PodGroupExplanation {
	...
	if len(pg.SkippedMigReasons) > 0 {
		explanation.SkippedMigs = make([]*vispb.MigExplanation, 0, len(pg.SkippedMigReasons))
		for _, migReason := range pg.SkippedMigReasons {
			explanation.SkippedMigs = append(explanation.SkippedMigs, migReason.Proto())
		}
	}
...
func (pg *PodGroupExplanation) Signature() PodGroupExplanationSignature {
	...
	sortedSkippedMigNames := make([]string, 0, len(pg.SkippedMigReasons))
	for migName := range pg.SkippedMigReasons {
		sortedSkippedMigNames = append(sortedSkippedMigNames, migName)
	}
	sort.Strings(sortedSkippedMigNames)

	var skippedMigsSignature strings.Builder
	for _, migName := range sortedSkippedMigNames {
		reason := pg.SkippedMigReasons[migName]
		skippedMigsSignature.WriteString(string(reason.Signature()))
		skippedMigsSignature.WriteRune('\n')
	}
	...
	return PodGroupExplanationSignature{
		...
		SkippedMigsSignature: skippedMigsSignature.String(),
	}
}

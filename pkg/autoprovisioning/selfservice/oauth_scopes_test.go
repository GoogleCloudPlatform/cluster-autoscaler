...
func TestOAuthScopes(t *testing.T) {
	...
	t.Run("FromCccSpec - empty OAuthScopes", func(t *testing.T) {
		spec := v1.ComputeClassSpec{
	...
	t.Run("FromCccSpec - with OAuthScopes", func(t *testing.T) {
		spec := v1.ComputeClassSpec{
			NodePoolConfig: &v1.NodePoolConfig{
				OAuthScopes: []string{
	...
	t.Run("FromCccSpec - with empty scope string", func(t *testing.T) {
		spec := v1.ComputeClassSpec{
			NodePoolConfig: &v1.NodePoolConfig{
				OAuthScopes: []string{""},
			},
	...
	t.Run("FromNodepool - empty OauthScopes", func(t *testing.T) {
		np := &container.NodePool{
	...
	t.Run("FromNodepool - with OauthScopes", func(t *testing.T) {
		np := &container.NodePool{
			Config: &container.NodeConfig{
				OauthScopes: []string{
	...
	t.Run("ToNodePoolLabels - no-op", func(t *testing.T) {
		...
		feature.ToNodePoolLabels(labels, Metadata{
			"oauth-scopes.cloud.google.com/scope": "true",
		})
	...
	t.Run("ToNodepool - empty metadata", func(t *testing.T) {
		...
		assert.Nil(t, np.Config.OauthScopes)
	})
	...
	t.Run("ToNodepool - non-true values ignored", func(t *testing.T) {
		...
		feature.ToNodepool(np, Metadata{
			"oauth-scopes.cloud.google.com/scope1": "false",
			"oauth-scopes.cloud.google.com/scope2": "other",
		})
		assert.Nil(t, np.Config.OauthScopes)
	})

	t.Run("ToNodepool - sets sorted OauthScopes on empty config", func(t *testing.T) {
		np := &container.NodePool{}
		feature.ToNodepool(np, Metadata{
			"oauth-scopes.cloud.google.com/scope-z": "true",
			"oauth-scopes.cloud.google.com/scope-a": "true",
			"oauth-scopes.cloud.google.com/scope-m": "true",
		})
		assert.NotNil(t, np.Config)
		assert.Equal(t, []string{"scope-a", "scope-m", "scope-z"}, np.Config.OauthScopes)
	})

	t.Run("ToNodepool - empty scope prefix ignored", func(t *testing.T) {
		np := &container.NodePool{
			Config: &container.NodeConfig{},
		}
		feature.ToNodepool(np, Metadata{
			"oauth-scopes.cloud.google.com/": "true",
		})
		assert.Nil(t, np.Config.OauthScopes)
	})

	t.Run("Unordered scopes between CCC spec and NodePool produce identical metadata and reconstruct sorted", func(t *testing.T) {
		spec := v1.ComputeClassSpec{
			NodePoolConfig: &v1.NodePoolConfig{
				OAuthScopes: []string{"scope-b", "scope-a"},
			},
		}
		np := &container.NodePool{
			Config: &container.NodeConfig{
				OauthScopes: []string{"scope-a", "scope-b"},
			},
		}
		...
		assert.Equal(t, []string{"scope-a", "scope-b"}, targetNp.Config.OauthScopes)
	})
}

...
func TestDeserializeKubeletConfig(t *testing.T) {
	for tn, tc := range map[string]struct {
		...
		"config with insecureKubeletReadonlyPortEnabled false": {
			kubeletConfig: `{"insecureKubeletReadonlyPortEnabled":false}`,
			expected: &gkeclient.NodeKubeletConfig{
...
func TestKubeletConfigSignature(t *testing.T) {
	for tn, tc := range map[string]struct {
		...
		"config with insecureKubeletReadonlyPortEnabled=false in forcesendfields": {
			...
			expected: "kubelet-config: <InsecureKubeletReadonlyPortEnabled: false>",
		},
	} {
		t.Run(tn, func(t *testing.T) {

...
func TestCSNPodsLifecycleProcess(t *testing.T) {
	testCases := []struct {
		...
		expectedMetrics           []metrics.CSNInvalidCondition
	}{
		{
...
func TestCSNPodsLifecycleProcess_ScheduleAndMarkSuspendableNodesError(t *testing.T) {
	csnNodes := []nodecontroller.CSNNode{
	...
	pod.UID = "uid-csn-p1"
	_, err := processor.scheduleAndMarkSuspendableNodes(clusterSnapshot, nil, []*apiv1.Pod{pod})
	assert.Error(t, err)
}

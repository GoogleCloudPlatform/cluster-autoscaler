...
func (p *CSNPodsLifecycleProcessor) Process(ctx *context.AutoscalingContext, unschedulablePods []*apiv1.Pod) ([]*apiv1.Pod, error) {
	...
	newUnschedulablePods, err := p.scheduleAndMarkSuspendableNodes(snapshot, unschedulablePods, csnPods)
	if err != nil {
		snapshot.Revert()
		klog.Errorf("%s error while scheduling CSN pods: %v", csnPodsLifecycleLogPrefix, err)
		p.metrics.SetCSNInvalidCondition(internalmetrics.CSNPodsLifecycleProcessorError)
		return unschedulablePods, nil // Snapshot is reverted, so we should return the original unschedulable pods.
	}
	if err := snapshot.Commit(); err != nil {
		klog.Errorf("%s error while commiting the snapshot: %v", csnPodsLifecycleLogPrefix, err)
	...
	return newUnschedulablePods, nil
}
...
func (p *CSNPodsLifecycleProcessor) scheduleAndMarkSuspendableNodes(snapshot clustersnapshot.ClusterSnapshot, unschedulablePods []*apiv1.Pod, csnPods []*apiv1.Pod) ([]*apiv1.Pod, error) {
	...
	suspendedNodes := p.nodeController.MarkAsSuspendable(slices.Collect(maps.Values(nodesToPotentiallySuspend)))

	for nodeName := range suspendedNodes {

...
func TestBufferConsumptionProcess(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			expectErr:                 true,
			expectedUnschedulablePods: []string{}, expectedAllConsumedNodes: nil,
		},
		{
			name: "Error from nodeController.Consume",
			initialNodes: []*apiv1.Node{
				create8CPUTestNode(t, "node-1", csn.NodeStateChilling),
			},
			unschedulablePods: []*apiv1.Pod{
			...
			consumeErr:                errors.New("can't consume nodes"),
			expectErr:                 true,
			expectedUnschedulablePods: []string{},
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			mockController.SetConsumeError(tc.consumeErr)

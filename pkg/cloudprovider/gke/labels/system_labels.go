...
// IsGkeDraNode tells if a given node has GKE DRA driver enabled, See go/dra-enablement-on-gke for more details.
func IsGkeDraNode(labels map[string]string) bool {
	for key := range labels {
		if strings.HasPrefix(key, "cloud.google.com/gke-") && strings.HasSuffix(key, "-dra-driver") {
			return true
		}
	}
	return false

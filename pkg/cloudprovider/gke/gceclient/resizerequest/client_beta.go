...
type resizeRequestClientBeta struct {
	...
	cache *resizeRequestOperationBetaCache
...
// NewResizeRequestClientBeta creates a new client to communicate with
// MIG Resize Request API. If gceEndpoint is not empty the base path is overridden.
func NewResizeRequestClientBeta(client *http.Client, projectID, userAgent, gceEndpoint string, resizeRequestMode ResizeRequestMode) (*resizeRequestClientBeta, error) {
	gceBetaService, err := gce_api_beta.New(client)
	if err != nil {
		return nil, err
	}
	...
	return &resizeRequestClientBeta{
		...
		cache:                        newResizeRequestOperationBetaCache(deleteOperationsCacheSize),
...
func (client *resizeRequestClientBeta) RegisterFailedResizeRequestsCreation(migRef gce.GceRef, err error, count int) {
...
func (client *resizeRequestClientBeta) ResetFailedResizeRequestsCreation(migRef gce.GceRef) map[error]int {
	return client.failedCreationRequestsPerMIG.reset(migRef)
}

// ResizeRequests lists all Resize Requests in the MIG.
func (client *resizeRequestClientBeta) ResizeRequests(ctx context.Context, migRef gce.GceRef) ([]ResizeRequestStatus, error) {
	items := []*gce_api_beta.InstanceGroupManagerResizeRequest{}
	start := time.Now()
	lastRequestStart := start
	err := client.gceBetaService.InstanceGroupManagerResizeRequests.List(migRef.Project, migRef.Zone, migRef.Name).Pages(
		ctx, func(gceResp *gce_api_beta.InstanceGroupManagerResizeRequestsListResponse) error {
	...
	for _, item := range items {
		rr, err := resizeRequestFromGCEBetaResponse(item, migRef)
		if err != nil {
...
func (client *resizeRequestClientBeta) ResizeRequest(ctx context.Context, migRef gce.GceRef, resizeRequestName string) (ResizeRequestStatus, error) {
	start := time.Now()
	gceResp, err := client.gceBetaService.InstanceGroupManagerResizeRequests.Get(migRef.Project, migRef.Zone, migRef.Name, resizeRequestName).Context(ctx).Do()
	...
	return resizeRequestFromGCEBetaResponse(gceResp, migRef)
}
...
func (client *resizeRequestClientBeta) AdvanceResizeRequestCleanUp(ctx context.Context, rr ResizeRequestStatus) error {
	...
	case cancelAction:
		...
		start := time.Now()
		operation, err := client.gceBetaService.InstanceGroupManagerResizeRequests.Cancel(rr.ProjectID, rr.Zone, rr.MigName, rr.Name).Context(ctx).Do()
	...
	case deleteAction:
		...
		start := time.Now()
		operation, err := client.gceBetaService.InstanceGroupManagerResizeRequests.Delete(rr.ProjectID, rr.Zone, rr.MigName, rr.Name).Context(ctx).Do()
...
func (client *resizeRequestClientBeta) ReportState(rr ResizeRequestStatus) ResizeRequestReportState {
	return client.cache.reportState(rr.ID)
}

// SetReportState sets the report state of the particular Resize Request
func (client *resizeRequestClientBeta) SetReportState(rr ResizeRequestStatus, state ResizeRequestReportState) {
	client.cache.setReportState(rr.ID, state)
}

// CreateResizeRequest creates a new Resize Request.
func (client *resizeRequestClientBeta) CreateResizeRequest(ctx context.Context, migRef gce.GceRef, createRequest ResizeRequestCreateRequest) error {
	gceResizeRequest := &gce_api_beta.InstanceGroupManagerResizeRequest{
	...
	if createRequest.RequestedRunDuration != nil {
		...
		gceResizeRequest.RequestedRunDuration = &gce_api_beta.Duration{
			Seconds: protoDuration.Seconds,
			Nanos:   int64(protoDuration.Nanos),
		}
	}
	start := time.Now()
	operation, err := client.gceBetaService.InstanceGroupManagerResizeRequests.Insert(migRef.Project, migRef.Zone, migRef.Name, gceResizeRequest).Context(ctx).Do()
	...
	return client.waitForBetaOp(operation, migRef.Project, migRef.Zone, gceResizeRequest.Name)
}
...
func (client *resizeRequestClientBeta) nextDeletionAction(rr ResizeRequestStatus) (nextAction, error) {
	...
	err := client.fetchBetaOp(op, rr.ProjectID, rr.Zone, rr.Name)
	switch {
...
func (client *resizeRequestClientBeta) waitForBetaOp(operation *gce_api_beta.Operation, projectID, zone, resizeRequestName string) error {
	start := time.Now()
	...
	for time.Since(start) < client.operationWaitTimeout {
		...
		err := client.fetchBetaOp(operation, projectID, zone, resizeRequestName)
...
// fetchBetaOp fetches the current state of the operation and depending on its status returns:
...
func (client *resizeRequestClientBeta) fetchBetaOp(operation *gce_api_beta.Operation, projectID, zone, resizeRequestName string) error {
	start := time.Now()
	op, err := client.gceBetaService.ZoneOperations.Get(projectID, zone, operation.Name).Do()
	...
	if op.Status == "DONE" {
		if op.Error != nil && op.Error.Errors != nil {
			...
			if op.OperationType == cancelOpType && hasConditionNotMetBetaErrorCode(op.Error.Errors) {
...
func resizeRequestFromGCEBetaResponse(rr *gce_api_beta.InstanceGroupManagerResizeRequest, migRef gce.GceRef) (ResizeRequestStatus, error) {
	...
	return ResizeRequestStatus{
		...
		Errors:               errorsFromGCEBeta(rr.Status.Error),
		LastAttemptErrors:    lastAttemptErrorsFromGCEBeta(rr.Status.LastAttempt),
	}, nil
}

func lastAttemptErrorsFromGCEBeta(gceLastAttempt *gce_api_beta.InstanceGroupManagerResizeRequestStatusLastAttempt) []DwsStatusError {
...
func errorsFromGCEBeta(gceError *gce_api_beta.InstanceGroupManagerResizeRequestStatusError) []DwsStatusError {
...
func hasConditionNotMetBetaErrorCode(errors []*gce_api_beta.OperationErrorErrors) bool {
	if errors == nil {
		return false
	}
...
// The cache is NOT THREAD SAFE (some methods do both acccess and write)
type resizeRequestOperationBetaCache struct {
	cache *lru.Cache
}

type rrBetaCacheOpStatus struct {
	op          *gce_api_beta.Operation
	reportState ResizeRequestReportState
}

func newResizeRequestOperationBetaCache(maxSize int) *resizeRequestOperationBetaCache {
	return &resizeRequestOperationBetaCache{
		cache: lru.New(maxSize),
	}
}

func (c *resizeRequestOperationBetaCache) operation(rrID uint64) *gce_api_beta.Operation {
	value, found := c.cache.Get(rrID)
	if !found {
		return nil
	}
	return value.(*rrBetaCacheOpStatus).op
}

func (c *resizeRequestOperationBetaCache) reportState(rrID uint64) ResizeRequestReportState {
	value, found := c.cache.Get(rrID)
	if !found {
		return UnspecifiedReportState
	}
	return value.(*rrBetaCacheOpStatus).reportState
}

func (c *resizeRequestOperationBetaCache) setOperation(rrID uint64, op *gce_api_beta.Operation) {
	c.cache.Add(rrID, &rrBetaCacheOpStatus{op, c.reportState(rrID)})
}

func (c *resizeRequestOperationBetaCache) setReportState(rrID uint64, state ResizeRequestReportState) {
	c.cache.Add(rrID, &rrBetaCacheOpStatus{c.operation(rrID), state})
}

func (c *resizeRequestOperationBetaCache) removeOperation(rrID uint64) {
	value, found := c.cache.Get(rrID)
	if found {
		state := value.(*rrBetaCacheOpStatus).reportState
		c.cache.Add(rrID, &rrBetaCacheOpStatus{nil, state})
	}
}

func (c *resizeRequestOperationBetaCache) remove(rrID uint64) {
	c.cache.Remove(rrID)
}

...
var zoneTypesEnabledField = trackedField{
	...
	getValueStr: func(opts internalopts.AutoscalingOptions) string {
		return fmt.Sprintf("%v", opts.ZoneTypesEnabled)
	},
	setValue: func(optsFromFlags internalopts.AutoscalingOptions, experimentsManager experiments.Manager, optsToModify *internalopts.AutoscalingOptions) error {
		// ZoneTypes::Enabled and ZoneTypes::MinCAVersion are not defined by default and only used in emergency management
		flagValue := optsFromFlags.ZoneTypesEnabled
		enabled := experimentsManager.EvaluateBoolFlagOrFailsafe(experiments.ZoneTypesEnabledFlag, flagValue)
		currentVersionSupported := experimentsManager.EvaluateMinimumVersionFlagOrFailsafe(experiments.ZoneTypesMinCAVersionFlag, flagValue)

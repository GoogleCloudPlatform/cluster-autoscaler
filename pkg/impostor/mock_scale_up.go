	...
	for _, nodeGroup := range nodeGroups {
		...
		for _, eg := range podEquivalenceGroups {
			...
			if err := context.ClusterSnapshot.CheckPredicates(samplePod, nodeInfo.Node().Name); err == nil {
				...
				klog.V(2).Infof("Pod %s can't be scheduled on %s, predicate failed: %v", samplePod.Name, nodeGroup.Id(), err)
		...
		if len(option.Pods) > 0 {
			builder, _ := estimator.NewEstimatorBuilder(
				...
				estimator.NewThresholdBasedEstimationLimiter([]estimator.Threshold{}),
			...
			// fmt.Printf("No pod can fit to %s\n", nodeGroup.Id())

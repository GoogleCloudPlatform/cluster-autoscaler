...
type ProvisioningRequestLister interface {
	...
	List(selector labels.Selector) (ret []*v1alpha1.ProvisioningRequest, err error)
...
// List lists all ProvisioningRequests in the indexer.
func (s *provisioningRequestLister) List(selector labels.Selector) (ret []*v1alpha1.ProvisioningRequest, err error) {
	err = cache.ListAll(s.indexer, selector, func(m interface{}) {
		ret = append(ret, m.(*v1alpha1.ProvisioningRequest))
	})
	return ret, err
}
...
type ProvisioningRequestNamespaceLister interface {
	...
	// Objects returned here must be treated as read-only.
	Get(name string) (*v1alpha1.ProvisioningRequest, error)
...
// List lists all ProvisioningRequests in the indexer for a given namespace.
func (s provisioningRequestNamespaceLister) List(selector labels.Selector) (ret []*v1alpha1.ProvisioningRequest, err error) {
...
// Get retrieves the ProvisioningRequest from the indexer for a given namespace and name.
func (s provisioningRequestNamespaceLister) Get(name string) (*v1alpha1.ProvisioningRequest, error) {
	obj, exists, err := s.indexer.GetByKey(s.namespace + "/" + name)
	if err != nil {
		return nil, err
	}
	if !exists {
		return nil, errors.NewNotFound(v1alpha1.Resource("provisioningrequest"), name)
	}
	return obj.(*v1alpha1.ProvisioningRequest), nil
}

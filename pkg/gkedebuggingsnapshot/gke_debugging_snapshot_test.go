...
func TestBasicSetterWorkflow(t *testing.T) {
	...
	nodeInfo := framework.NewTestNodeInfo(nil, pod)

	capacityRequest := []*cr_v1.CapacityRequest{
		{
			Status: cr_v1.CapacityRequestStatus{},
			ObjectMeta: metav1.ObjectMeta{
				Name: "capacity-request-1",
			},
		},
	}
	...
	snapshot.SetCapacityRequest(capacityRequest)
	...
	assert.IsType(t, JSONList{}, parsed["CapacityRequest"])
	assert.Greater(t, len(parsed["CapacityRequest"].([]any)), 0)
	assert.IsType(t, JSONMap{}, parsed["CapacityRequest"].([]any)[0])
	cr := parsed["CapacityRequest"].([]any)[0].(map[string]any)
	assert.IsType(t, JSONMap{}, cr["metadata"].(map[string]any))
	crObjectMeta := cr["metadata"].(map[string]any)
	assert.IsType(t, String, crObjectMeta["name"])
	crName := crObjectMeta["name"].(string)
	assert.Equal(t, crName, "capacity-request-1")
}

...
func (g *instanceConfigGenerator) generateInstanceConfigs(flexibilityScopeKey string) (*GeneratorArtifacts, []error) {
	...
	for idx, ruleGroup := range crd.GroupedRules() {
		for _, rule := range ruleGroup {
			...
			instanceConfigsForRule, errorsForRule := g.generateInstanceConfigsForRule(rule, rank)
			allErrors = append(allErrors, errorsForRule...)

			g.validateGeneratedConfigsForRule(flexibilityScopeKey, instanceConfigsForRule, errorsForRule)
...
// generateInstanceConfigsForRule generates all possible instance configurations (InstanceConfig) for a single rule.
func (g *instanceConfigGenerator) generateInstanceConfigsForRule(rule rules.Rule, rank int) ([]*api.InstanceConfig, []error) {
	if len(rule.NodePoolNames()) > 0 {
		configs, errs := g.instanceConfigsForNodePools(rule, rank)
		return configs, errs
	}

	configs, err := g.deriveMachineConfigsFromRule(rule, rank)
	if err != nil {
		return nil, []error{fmt.Errorf("error when generating instance configs: %v", err)}
	}
	configs = g.expandConfigsByProvisioningMode(rule, configs)
	configs = g.expandConfigsByMaxRunDuration(rule, configs)
	var zoneErrors []error
	configs, zoneErrors = g.assignZonesToConfigs(rule, configs)
	return configs, zoneErrors
}

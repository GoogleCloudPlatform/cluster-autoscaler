...
func (descriptor *NodeGroupDescriptor) signature() ShardSignature {
	...
	if descriptor.ProvisioningCapacitySearchStrategy != "" {
		builder.WriteString(fmt.Sprintf("ProvisioningCapacitySearchStrategy(%s)", descriptor.ProvisioningCapacitySearchStrategy))
	}
	if descriptor.CSNBufferID != "" {
		builder.WriteString(fmt.Sprintf("CSNBufferID(%s)", descriptor.CSNBufferID))
	}
	if descriptor.RequiresGPU {
		builder.WriteString("RequiresGPU(true)")
	}
	if builder.Len() == 0 {
		return "_"
	}
...
func (descriptor NodeGroupDescriptor) DeepCopy() NodeGroupDescriptor {
	...
	copy.ProvisioningCapacitySearchStrategy = descriptor.ProvisioningCapacitySearchStrategy
	copy.CSNBufferID = descriptor.CSNBufferID
	copy.RequiresGPU = descriptor.RequiresGPU
	return copy
}

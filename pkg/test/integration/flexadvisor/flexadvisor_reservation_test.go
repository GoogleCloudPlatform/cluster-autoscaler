...
func TestReservationAndFAReturnNoCapacitySinglePod(t *testing.T) {
	...
	for name, tt := range map[string]struct {
		...
		"reservation_on_unknown_by_fa_machine_type": {
			ccc: ccc.NewComputeClassBuilder("test-ccc").WithNodePoolsRules("pool-1").Build(),
			nodePools: []*gke_api_beta.NodePool{
				integration.EmptyNodePool("pool-1").WithMachineType(UnknownAvailabilityMachineType).WithCCCLabel("test-ccc").Build(),
...
func TestReservationEnforcesTwoStepAllocation(t *testing.T) {
	...
	nodePools := []*gke_api_beta.NodePool{
		integration.EmptyNodePool("pool-1").WithMachineType(OneInstanceAvailableMachineType).WithCCCLabel(crd.Name).Build(),
		integration.EmptyNodePool("pool-2").WithMachineType(StockOutMachineType).WithCCCLabel(crd.Name).Build(),
	}

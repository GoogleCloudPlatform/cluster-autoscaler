...
func TestFlexAdvisorFailedToRespond(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		infra.Fakes.FlexAdvisorClient.AddCapacityGuidances(fake.FakeCapacityGuidance{Error: errors.New("FA internal error")})
	...
	synctest.Test(t, func(t *testing.T) {
		...
		infra.Fakes.FlexAdvisorClient.AddCapacityGuidances(
			fake.FakeCapacityGuidance{
				MachineType: new(OneInstanceAvailableMachineType),
				Zone:        new(ZoneB),
				Omit:        true,
			},
...
func TestFlexAdvisorTimeout(t *testing.T) {
	nodePools := []*gke_api_beta.NodePool{
		// due to time-out FA recommendation should not be taken into account
		createEmptyNodePool("pool-1", ZeroCapacityRecommendationMachineType),
		createEmptyNodePool("pool-2", AvailableMachineType),
	}
	ccc := createCCCWithNodePoolsRules([]string{"pool-1", "pool-2"})
...
func TestFlexAdvisorMissingRecommendationForMachineType(t *testing.T) {
	...
	synctest.Test(t, func(t *testing.T) {
		...
		infra.Fakes.FlexAdvisorClient.AddCapacityGuidances(
			fake.FakeCapacityGuidance{
				MachineType: new(UnknownAvailabilityMachineType),
				Omit:        true,
			},
			fake.NewFakeCapacityGuidanceForMachineType(AvailableMachineType, 10, 0.5),
		)
...
func TestFlexAdvisorSpotVsOnDemand(t *testing.T) {
	...
	nodePools := []*gke_api_beta.NodePool{
		integration.DefaultNodePool(
			integration.WithNodePoolName(spotPoolName),
			integration.WithNodePoolMachineType(machineType),
			integration.WithNodePoolSize(0),
			integration.WithNodePoolLocations(ZoneB),
			integration.WithNodePoolSpot(),
		),
		integration.DefaultNodePool(
			integration.WithNodePoolName(standardPoolName),
			integration.WithNodePoolMachineType(machineType),
			integration.WithNodePoolSize(0),
			integration.WithNodePoolLocations(ZoneB),
		),
	}
	ccc := createCCCWithNodePoolsRules([]string{spotPoolName, standardPoolName})
	nodePools = annotateNodePoolWithCCCLabel(ccc.Name, nodePools)
	...
	synctest.Test(t, func(t *testing.T) {
		...
		pod := tu.BuildTestPod("spot-pod", 3000, 12000, tu.MarkUnschedulable(), withTestCCC)

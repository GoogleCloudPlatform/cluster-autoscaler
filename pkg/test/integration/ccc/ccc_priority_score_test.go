...
func TestCCCScaleUpPriorityScore(t *testing.T) {
	testCases := []struct {
	...
	}{
		{
			...
			ccc: ccc.NewComputeClassBuilder("scaleup-priorityscore-prefer-higher").
		...
		},
		{
			...
			ccc: ccc.NewComputeClassBuilder("scaleup-priorityscore-equal").
				WithNodePoolAutoCreation(true).
				WithPriorities(
					v1.Priority{
						MachineFamily: ptr.To("e2"),
...
func TestCCCDefragPriorityScore(t *testing.T) {
	baseAdd := ccc.NewComputeClassBuilder("defrag-priorityscore-add").
		WithNodePoolAutoCreation(true).
	...
	baseChange := ccc.NewComputeClassBuilder("defrag-priorityscore-change").
		WithNodePoolAutoCreation(true).
		WithActiveMigration(true)
	...
	for _, tc := range testCases {
		t.Run(tc.name, func(t *testing.T) {
			...
			synctest.Test(t, func(t *testing.T) {
				...
				integration_synctest.MustRunOnceAfter(ctx, t, autoscaler, 5*time.Minute+time.Second)
				...
				integration_synctest.MustRunOnceAfter(ctx, t, autoscaler, 15*time.Minute)
				integration_synctest.MustRunOnceAfter(ctx, t, autoscaler, time.Second)
